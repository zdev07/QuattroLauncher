...
/** * Quattro Progress Layout
 * Monitors background tasks and displays them to the user.
 */
public class ProgressLayout extends ConstraintLayout implements View.OnClickListener, TaskCountListener {
    ...
    public ProgressLayout(@NonNull Context context) { super(context); init(); }
    public ProgressLayout(@NonNull Context context, @Nullable AttributeSet attrs) { super(context, attrs); init(); }
    public ProgressLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) { super(context, attrs, defStyleAttr); init(); }
    ...
    public static void clearProgress(String progressKey) {
        ProgressKeeper.submitProgress(progressKey, -1, -1, (Object)null);
    }

    @Override
    public void onClick(View v) {
        boolean isVisible = mLinearLayout.getVisibility() == VISIBLE;
        mLinearLayout.setVisibility(isVisible ? GONE : VISIBLE);
        mFlipArrow.setRotation(isVisible ? 0 : 180);
    }
    ...
    public void onUpdateTaskCount(int tc) {
        post(() -> {
            if (tc > 0) {
                // Displays "Quattro: X tasks in progress"
    ...
    class LayoutProgressListener implements ProgressListener {
        ...
        public void onProgressStarted() {
            post(() -> mLinearLayout.addView(textView, params));
        }
